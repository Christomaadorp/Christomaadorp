        ...
        <div class="col-lg-12">
            <div class="panel panel-default">
                <div class="panel-heading">
                    {{ trans('global.edit') }} {{ trans('cruds.taskResult.title_singular') }}
                </div>
                <div class="panel-body">
                    <form method="POST" action="{{ route("admin.task-results.update", [$taskResult->id]) }}" enctype="multipart/form-data">
                        @method('PUT')
                        @csrf
                        <div class="form-group {{ $errors->has('taskid') ? 'has-error' : '' }}">
                            <label for="taskid">{{ trans('cruds.taskResult.fields.taskid') }}</label>
                            <input class="form-control" type="number" name="taskid" id="taskid" value="{{ old('taskid', $taskResult->taskid) }}" step="1">
                        ...
                        <div class="form-group {{ $errors->has('runat') ? 'has-error' : '' }}">
                            <label for="runat">{{ trans('cruds.taskResult.fields.run_at') }}</label>
                            <input class="form-control timepicker" type="text" name="runat" id="runat" value="{{ old('runat', $taskResult->runat) }}">
                            @if($errors->has('runat'))
                        ...
                        <div class="form-group {{ $errors->has('duration') ? 'has-error' : '' }}">
                            ...
                            <input class="form-control" type="text" name="duration" id="duration" value="{{ old('duration', $taskResult->duration) }}">
                        ...
                        <div class="form-group {{ $errors->has('result') ? 'has-error' : '' }}">
                            ...
                            <input class="form-control" type="text" name="result" id="result" value="{{ old('result', $taskResult->result) }}">
                        ...
                        <div class="form-group {{ $errors->has('timestamp') ? 'has-error' : '' }}">
                            ...
                            <input class="form-control timepicker" type="text" name="timestamp" id="timestamp" value="{{ old('timestamp', $taskResult->timestamp) }}">
